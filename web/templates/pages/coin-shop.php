        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-4">
            <?php foreach($packages as $pkg): ?>
                <div class="bg-gray-900 border border-gray-800 rounded-lg p-6 text-center hover:scale-105 transition cursor-pointer relative overflow-hidden flex flex-col h-full shadow-lg">
                    ...
                    <div class="absolute top-0 left-0 right-0 h-2" style="background-color: <?= htmlspecialchars($pkg['color_code'] ?? '#333') ?>;"></div>

                    <h3 class="text-xl font-bold mb-4 mt-2" style="color: <?= htmlspecialchars($pkg['color_code'] ?? '#fff') ?>;"><?= htmlspecialchars($pkg['name']) ?></h3>
                    ...
                    <form action="/coins/purchase" method="POST" class="mt-auto">
                        ...
                        <button type="submit" class="w-full text-white font-bold py-3 px-4 rounded transition-colors shadow-lg hover:brightness-110" style="background-color: <?= htmlspecialchars($pkg['color_code'] ?? '#dc2626') ?>; color: #000;">
