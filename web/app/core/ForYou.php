 ...
 */
class ForYou {
    /** @return array<int, array{episode_id:int, trailer_url:string, series_id:int, series_title:string, cover_image:?string}> */
    public static function randomTrailers(PDO $db, int $limit = 20): array {
        ...
        $stmt = $db->prepare("
            SELECT e.id AS episode_id, e.trailer_url, e.series_id, s.title AS series_title, s.cover_image
            FROM episodes e
